export function tokenizeTrack(track, langDef, sDef) {
  const stateStore = [[]]
  const states = ['root']
  const sfStates = [0]
...
export function mergeSimplifiedFunc(state, count, langDef, sDef) {
  if (count === 0) return
  let lastCount = -1
...
export function compare(pattern, patternLength, state, startIndex) {
  let count = 0
...
export function sameType(pat, sta) {
